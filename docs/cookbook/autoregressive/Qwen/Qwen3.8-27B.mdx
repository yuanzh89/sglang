...
# so pull a main-tracking build instead:
...
<Warning>
**DFLASH2 needs a build that tracks main.** The DFlash2 selector reached main in
[#35496](https://github.com/sgl-project/sglang/pull/35496), which is newer than
both the pinned `lmsysorg/sglang:qwen38-27b` tag and the latest PyPI release. On
the NVFP4 checkpoint an older build fails at boot with `requires a dense
FP16/BF16/FP32 target lm_head`. Every other recipe on this page — no speculation,
MTP, DSpark — runs on the pinned tag as written.
</Warning>
...
<Warning>
  **Picking DFLASH2 requires SGLang built from main.** The DFlash2 selector
  landed in [#35496](https://github.com/sgl-project/sglang/pull/35496), which is
  newer than the `lmsysorg/sglang:qwen38-27b` tag pinned in
  [Install SGLang](#install) and newer than the latest PyPI release. On the
  NVFP4 checkpoint an older build fails at boot with `requires a dense
  FP16/BF16/FP32 target lm_head`. Every other Speculative Decoding pick — None,
  EAGLE, DSPARK — runs on the pinned tag as written.
</Warning>
